...
class HighlightAnalysisStep(PipelineStep):
    ...
    def _resolve_color_output_path(self):
        ...
        if output_path:
            ...
            if 'highlight' in lowered:
                name = name[: lowered.rfind('highlight')].rstrip('_-.') or 'mode_color'
    ...
    def _resolve_base_output_path(self):
        ...
        return color_output_path.with_name(f"{color_output_path.stem}_highlight{suffix}")
    ...
    def _build_highlight_mask(self, region_mask, l_channel):
        l_values = l_channel[region_mask].astype(np.float32)
        if l_values.size == 0:
            return region_mask & False, 0.0, 0.0

        highlight_percentile = float(self.params.get('highlight_percentile', 0.3))
        highlight_percentile = min(max(highlight_percentile, 0.0), 1.0)
        cutoff = float(np.percentile(l_values, (1.0 - highlight_percentile) * 100.0))
        highlight_mask = region_mask & (l_channel.astype(np.float32) >= cutoff)

        region_pixels = max(1, int(region_mask.sum()))
        highlight_ratio = float(highlight_mask.sum()) / float(region_pixels)
        return highlight_mask, cutoff, highlight_ratio
    ...
    def _apply_highlight_morphology(self, highlight_mask):
        morph_cfg = self.params.get('morphology', {}) or {}
        enabled = bool(morph_cfg.get('enabled', False))
        if not enabled:
            return highlight_mask, []

        processed = highlight_mask.astype(np.uint8) * 255
        operations = []
    ...
    def _build_region_highlight_output_path(self, region_path, base_output_path, multiple_regions):
        if not multiple_regions:
            return str(base_output_path)
        region_path = Path(region_path)
        return str(region_path.with_name(f"{region_path.stem}_highlight{region_path.suffix or base_output_path.suffix or '.png'}"))

    def _build_region_source_highlight_output_path(self, region_path, source_color_output_path, multiple_regions):
        if not multiple_regions:
            return str(source_color_output_path)
        region_path = Path(region_path)
        suffix = region_path.suffix or source_color_output_path.suffix or '.png'
        return str(region_path.with_name(f"{region_path.stem}_highlight_source{suffix}"))
    ...
    def _build_region_source_gray_overlay_output_path(self, region_path, gray_overlay_output_path, multiple_regions):
        ...
        return str(region_path.with_name(f"{region_path.stem}_highlight_source_gray_overlay{suffix}"))

    def _build_gray_overlay_image(self, source_image, source_highlight_image):
        ...
        mask = np.any(source_highlight_image > 0, axis=2)
        if np.any(mask):
            overlay_layer[mask] = source_highlight_image[mask]
    ...
    def _analyze_single_region(self, mask_image, working_image, source_image):
        ...
        highlight_mask, cutoff, highlight_ratio = self._build_highlight_mask(region_mask, l_channel)
        highlight_mask, morphology_ops = self._apply_highlight_morphology(highlight_mask)
        highlight_ratio = float(highlight_mask.sum()) / float(max(1, int(region_mask.sum())))

        mode_result = np.zeros_like(source_image)
        source_result = np.zeros_like(source_image)
        if np.any(highlight_mask):
            mode_color = self._compute_mode_color(source_image, highlight_mask)
            mode_result[highlight_mask] = mode_color
            source_result[highlight_mask] = source_image[highlight_mask]
        else:
            mode_color = [0, 0, 0]

        stats = {
            'region_pixels': int(region_mask.sum()),
            'highlight_pixels': int(highlight_mask.sum()),
    ...
    def process(self, image):
        ...
        if not region_paths:
            ...
            self.params['last_highlight_region_paths'] = []
            self.params['last_highlight_source_region_paths'] = []
            self.params['last_highlight_gray_overlay_region_paths'] = []
            self.params['last_highlight_regions'] = []
            return working_image

        multiple_regions = len(region_paths) > 1
        combined_highlight = np.zeros_like(source_image)
        combined_source_highlight = np.zeros_like(source_image)
        ...
        for idx, region_path in enumerate(region_paths):
            ...
            highlight_image, source_highlight_image, stats = self._analyze_single_region(mask_image, working_image, source_image)
            region_output_path = self._build_region_highlight_output_path(region_path, base_output_path, multiple_regions)
            region_source_output_path = self._build_region_source_highlight_output_path(region_path, source_color_output_path, multiple_regions)
            ...
            region_gray_overlay_image = self._build_gray_overlay_image(source_image, source_highlight_image)
            cv2.imwrite(region_output_path, highlight_image)
            cv2.imwrite(region_source_output_path, source_highlight_image)
            ...
            highlight_pixels = np.any(highlight_image > 0, axis=2)
            if np.any(highlight_pixels):
                combined_highlight[highlight_pixels] = highlight_image[highlight_pixels]
            source_highlight_pixels = np.any(source_highlight_image > 0, axis=2)
            if np.any(source_highlight_pixels):
                combined_source_highlight[source_highlight_pixels] = source_highlight_image[source_highlight_pixels]
                combined_gray_overlay[source_highlight_pixels] = source_highlight_image[source_highlight_pixels]
        ...
        if multiple_regions:
            cv2.imwrite(str(base_output_path), combined_highlight)
            cv2.imwrite(str(source_color_output_path), combined_source_highlight)
        ...
        self.params['last_highlight_region_paths'] = saved_paths
        self.params['last_highlight_source_region_paths'] = saved_source_paths
        self.params['last_highlight_gray_overlay_region_paths'] = saved_gray_overlay_paths
        self.params['last_highlight_regions'] = highlight_regions
        return working_image
